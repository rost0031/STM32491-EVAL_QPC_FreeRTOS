...
#define QTimeEvt_postIn(me_, act_, nTicks_) do { \
    ...
    QTimeEvt_arm_((me_), (act_), (nTicks_), __FILE__, __LINE__); \
} while (0)
...
void QF_onIdle(void);
    ...
    */
    void QF_publish(QEvt const * const e, void const * const sender, char* filex, int line);
    ...
    * \sa ::QF_publish().
    */
    #define QF_PUBLISH(e_, sender_)  QF_publish((e_), (void const *)(sender_), __FILE__, __LINE__ )

#else

    /* Defined a new version that takes in file and line number for debugging
    void QF_publish(QEvt const * const e);
    #define QF_PUBLISH(e_, dummy_)   QF_publish(e_)
     */
    void QF_publish(QEvt const * const e,  char* filex, int line);
    #define QF_PUBLISH(e_, dummy_)   QF_publish(e_, __FILE__, __LINE__ )
...
/*    QEvt *QF_new_(QEvtSize const evtSize, enum_t const sig);
    ...
    QEvt *QF_new_(QEvtSize const evtSize, enum_t const sig, char* filex, int line);

#define Q_NEW(evtT_, sig_) \
        ((evtT_ *)QF_new_((QEvtSize)sizeof(evtT_), (sig_), __FILE__, __LINE__))
